...
fn write_goal(
    ...
    if let Some(d) = &g.due {
        ...
        match (*d - *today).whole_days() {
            i @ 2..=i64::MAX => { write!(&mut due_from, "in {} days", i).map_err(|e| e.to_string())?; },
            1 => { write!(&mut due_from, "tomorrow").map_err(|e| e.to_string())?; },
            0 => { write!(&mut due_from, "today").map_err(|e| e.to_string())?; },
            -1 => { write!(&mut due_from, "yesterday").map_err(|e| e.to_string())?; }
            i @ i64::MIN..=-2 => { write!(&mut due_from, "{} days ago", -i).map_err(|e| e.to_string())?; },
        }
    }
    ...
    if let (Some(d), Some(n)) = (&g.due, &g.done) {
        match (*d - *n).whole_days() {
            i @ 2..=i64::MAX => { write!(&mut done_from, "{} days early", &i).map_err(|e| e.to_string())?; },
            1 => { write!(&mut done_from, "one day early").map_err(|e| e.to_string())?; },
            0 => { write!(&mut done_from, "on time").map_err(|e| e.to_string())?; },
            -1 => { write!(&mut done_from, "one day late").map_err(|e| e.to_string())?; },
            i @ i64::MIN..=-2 => { write!(&mut done_from, "{} days late", -i).map_err(|e| e.to_string())?; },
        }
    }
...
fn write_summary(
    ...
    write_template("summary_row", &data, buff).map_err(|e| format!(
...
pub async fn login(
    ...
    let rows = match String::from_utf8(goals_buff) {
        ...
        Err(e) => {
            log::error!(
                "Buffer of Goal lines for some reaosn not UTF-8: {}", &e
            );
            return html_500();
        }
    };
